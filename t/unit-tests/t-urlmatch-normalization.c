...
static void check_url_normalizable(const char *url, int normalizable)
{
	...
	check_int(normalizable, ==, url1 ? 0 : 1);
	free(url1);
}
				    ...
				    size_t equal)
{
	...
	if (equal)
		...
		if(!check_int(strcmp(url1_norm, url2_norm), !=, 0))
			test_msg("url1_norm: %s\n  url2_norm: %s", url1_norm, url2_norm);
	free(url1_norm);
	free(url2_norm);
}
...
static void check_normalized_url_from_file(const char *file, const char *expect)
{
	...
	check_normalized_url(content.buf, expect);
cleanup:
...
{
	check_url_normalizable("", 1);
	check_url_normalizable("_", 1);
	check_url_normalizable("scheme", 1);
	check_url_normalizable("scheme:", 1);
	check_url_normalizable("scheme:/", 1);
	check_url_normalizable("scheme://", 1);
	...
	check_url_normalizable("://acme.co", 1);
	check_url_normalizable("x_test://acme.co", 1);
	check_url_normalizable("-test://acme.co", 1);
	check_url_normalizable("0test://acme.co", 1);
	check_url_normalizable("+test://acme.co", 1);
	check_url_normalizable(".test://acme.co", 1);
	check_url_normalizable("schem%6e://", 1);
	check_url_normalizable("x-Test+v1.0://acme.co", 0);
	check_normalized_url("AbCdeF://x.Y", "abcdef://x.y/");
}

static void t_url_authority(void)
{
	check_url_normalizable("scheme://user:pass@", 1);
	check_url_normalizable("scheme://?", 1);
	check_url_normalizable("scheme://#", 1);
	check_url_normalizable("scheme:///", 1);
	check_url_normalizable("scheme://:", 1);
	check_url_normalizable("scheme://:555", 1);
	check_url_normalizable("file://user:pass@", 0);
	check_url_normalizable("file://?", 0);
	check_url_normalizable("file://#", 0);
	check_url_normalizable("file:///", 0);
	check_url_normalizable("file://:", 0);
	check_url_normalizable("file://:555", 1);
	check_url_normalizable("scheme://user:pass@host", 0);
	check_url_normalizable("scheme://@host", 0);
	check_url_normalizable("scheme://%00@host", 0);
	check_url_normalizable("scheme://%%@host", 1);
	check_url_normalizable("scheme://host_", 0);
	check_url_normalizable("scheme://user:pass@host/", 0);
	check_url_normalizable("scheme://@host/", 0);
	check_url_normalizable("scheme://host/", 0);
	check_url_normalizable("scheme://host?x", 0);
	check_url_normalizable("scheme://host#x", 0);
	check_url_normalizable("scheme://host/@", 0);
	check_url_normalizable("scheme://host?@x", 0);
	check_url_normalizable("scheme://host#@x", 0);
	check_url_normalizable("scheme://[::1]", 0);
	check_url_normalizable("scheme://[::1]/", 0);
	check_url_normalizable("scheme://hos%41/", 1);
...
{
	check_url_normalizable("xyz://q@some.host:", 0);
	check_url_normalizable("xyz://q@some.host:456/", 0);
	check_url_normalizable("xyz://q@some.host:0", 1);
	check_url_normalizable("xyz://q@some.host:0000000", 1);
	check_url_normalizable("xyz://q@some.host:0000001?", 0);
	check_url_normalizable("xyz://q@some.host:065535#", 0);
	check_url_normalizable("xyz://q@some.host:65535", 0);
	check_url_normalizable("xyz://q@some.host:65536", 1);
	check_url_normalizable("xyz://q@some.host:99999", 1);
	check_url_normalizable("xyz://q@some.host:100000", 1);
	check_url_normalizable("xyz://q@some.host:100001", 1);
	check_url_normalizable("http://q@some.host:80", 0);
	check_url_normalizable("https://q@some.host:443", 0);
	check_url_normalizable("http://q@some.host:80/", 0);
	check_url_normalizable("https://q@some.host:443?", 0);
	check_url_normalizable("http://q@:8008", 1);
	check_url_normalizable("http://:8080", 1);
	check_url_normalizable("http://:", 1);
	check_url_normalizable("xyz://q@some.host:456/", 0);
	check_url_normalizable("xyz://[::1]:456/", 0);
	check_url_normalizable("xyz://[::1]:/", 0);
	check_url_normalizable("xyz://[::1]:000/", 1);
	check_url_normalizable("xyz://[::1]:0%300/", 1);
	check_url_normalizable("xyz://[::1]:0x80/", 1);
	check_url_normalizable("xyz://[::1]:4294967297/", 1);
	check_url_normalizable("xyz://[::1]:030f/", 1);
}
...
{
	check_url_normalizable("http://x.y?%fg", 1);
	check_normalized_url("X://W/%7e%41^%3a", "x://w/~A%5E%3A");
	check_normalized_url("X://W/:/?#[]@", "x://w/:/?#[]@");
	check_normalized_url("X://W/$&()*+,;=", "x://w/$&()*+,;=");
	check_normalized_url_from_file("url-13", "x://w/'%5C''");
	check_normalized_url_from_file("url-12", "x://w/?'%5C!");
}
...
static void t_url_dots(void)
{
	...
	check_url_normalizable("x://y/a/./b/.././../c/././.././..", 1);
	check_normalized_url("x://y/a/./?/././..", "x://y/a/?/././..");
