...
static int init_hash_algo(void)
{
	...
	if (algo < 0) {
		...
		if (!check(algo != GIT_HASH_UNKNOWN))
			test_msg("BUG: invalid GIT_TEST_DEFAULT_HASH value ('%s')", algo_name);
	}
	return algo;
}
				       ...
				       const struct git_hash_algo *algop)
{
	...
	if (!check_int(ret, ==, 0))
		test_msg("BUG: invalid hex input provided");
...
int get_oid_arbitrary_hex(const char *hex, struct object_id *oid)
{
	...
	if (!check_int(hash_algo, !=, GIT_HASH_UNKNOWN))
		return -1;
	return get_oid_arbitrary_hex_algop(hex, strlen(hex), oid,
